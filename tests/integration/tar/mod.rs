...
use anyhow::Result;
use oci2git::sources::{TarSource, Source};
...
mod tests {
    ...
    fn test_tar_source_with_existing_file() -> Result<()> {
        ...
        assert!(result.is_ok(), "Should successfully handle existing tar file");
        
        let (tarball_path, temp_dir) = result.unwrap();
        
        // For tar source, it should return the original path
        assert_eq!(tarball_path.to_str().unwrap(), temp_path, "Should return original tar file path");
        
        // No temp directory should be created for existing files
        assert!(temp_dir.is_none(), "No TempDir should be created for existing files");
    ...
    fn test_tar_source_with_nonexistent_file() {
        ...
        match result {
            Ok((path, _)) => {
                assert_eq!(path.to_str().unwrap(), nonexistent_path, 
